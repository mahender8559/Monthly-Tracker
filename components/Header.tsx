...
interface HeaderProps { selectedMonth: string; months: string[]; cycleStartDay: number; cycleLabel: string; userEmail?: string; onMonthChange: (month: string) => void; onCycleStartDayChange: (day: number) => void; onRollover: () => void; onSignOut: () => void; }

export function Header({ selectedMonth, months, cycleStartDay, cycleLabel, userEmail, onMonthChange, onCycleStartDayChange, onRollover, onSignOut }: HeaderProps) {
  return <header className="relative mb-6 overflow-hidden rounded-2xl border border-indigo-900/50 bg-gradient-to-r from-slate-900 via-indigo-900 to-slate-900 px-6 py-8 text-white shadow-xl md:mb-8 md:px-7 md:py-10">
    ...
    <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
      <div className="text-center md:text-left"><h1 className="bg-gradient-to-r from-white to-blue-200 bg-clip-text text-3xl font-black tracking-tight text-transparent sm:text-4xl">Financial Overview</h1><p className="mt-1 text-sm font-medium text-indigo-200 sm:text-base">Track, manage, and grow your wealth.</p>{userEmail && <p className="mt-2 text-xs font-semibold text-indigo-100">Signed in as {userEmail}</p>}</div>
